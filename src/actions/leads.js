...
export const getLeads = () => (dispatch, getState) => {
    ...
    axios
        .get(fetchurl+"api/bookmarks/", tokenConfig(getState))
        .then(res => {
            dispatch({
                type: GET_LEADS,
                payload: res.data['results']
...
export const deleteLead = id => (dispatch, getState) => {
    axios
        .delete(fetchurl+`api/bookmarks/${id}`, tokenConfig(getState))
        .then(res=>{
...
export const addLead = lead => (dispatch, getState) => {
    ...
    axios
        .post(fetchurl+"api/bookmarks/", lead, tokenConfig(getState))
        .then(res=>{
            console.log(res)
            dispatch(createMessage({
...
export const makePayment = lead => (dispatch, getState) => {
    const mykeyid = "rzp_test_egWWu3rbMpGA8Z";
    const myKeySecret = "H0SV22ONrpy2jWrjU7GXbMRi";
    console.log("In payments...")
    axios
        .post(`https://${mykeyid}:${myKeySecret}@api.razorpay.com/v1/payments/pay_29QQoUBi66xm2f/capture`, lead, tokenConfig(getState))
        .then(res=> {
            console.log("RES = "+res);
            dispatch(createMessage({
                addLead: "PAYMENT BEING DONE"
            }))
        })
        .catch(err => 
                console.log("ERROR OCCUREED IS = "+ str(err))
            )


        // fetchurl+`api/bookmarks/${id}`
}
